...
function buildReport(models, source) {
  const sections = Object.entries(models).map(([name,d]) => {
    const typical = Number(d.medianSold ?? d.avgSold ?? 0);
    const p10 = Number(d.p10 ?? typical);
    const p90 = Number(d.p90 ?? typical);
    const confidence = String(d.confidence ?? 'unknown').toUpperCase();
    const maxBuy = typical ? typical * (confidence === 'HIGH' ? 0.55 : confidence === 'MEDIUM' ? 0.52 : 0.48) : 0;
