...
const patchedMonitor = monitorSource
  // IMPORTANT: compare every candidate against the frontier from the PREVIOUS scan,
  // not against a maxId that changes while iterating the current result page.
  .replace(
  ...
  // Let missing sizes be recovered from the actual listing detail page before rejecting.
  .replace("const size = inferSize(item.fullText, targetSizes);", "let size = inferSize(item.fullText, targetSizes);")
  .replace(
  ...
  // Unknown condition is allowed when there is no explicit bad-condition signal.
  .replace(
    /if \(condition === 'unknown'\) \{/g,
    "if (condition === 'unknown') { condition = 'newWithoutTags'; }\n      if (condition === 'unknown') {"
  )
  // Re-check useful previously-blocked listings after filter changes.
  .replace(
  ...
  // Persist a tiny silent diagnostic so we can prove scans are happening and see alert count.
  .replace(
...
console.log('Freshness bug fix active: candidates compare against previous-scan frontier.');
console.log('Missing-size detail recovery active.');
