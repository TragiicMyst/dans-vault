...
const UA = 'Mozilla/5.0 (compatible; DansVaultSellerRisk/3.0; +https://github.com/TragiicMyst/dans-vault)';
const now = new Date();
if (TEST_MODE) {
  await send(webhook,{level:'🧪 TEST',seller:'Webhook connection test',model:'Nike test listing',price:25,url:'https://www.vinted.co.uk/',flags:['Discord webhook connected','Seller Risk monitor is running'],candidateCount:1,lowShare:0,modelCount:1});
  process.exit(0);
}

const allSearches = CONFIG.searches ?? [];
const perRun = Math.max(1, Math.min(Number(process.env.SELLER_RISK_SEARCHES_PER_RUN ?? 5), allSearches.length || 1));
...
let scanned=0,sellerFound=0,alerts=0;

for (const search of searches) {
  let html='';
  try { html=await fetchText(search.buyUrl); } catch(e){ console.warn(`${search.name}: ${e.message}`); continue; }
  for (const item of extractItems(html, 25)) {
    scanned++;
    ...
    const flags=[];
    if(/\b(?:1:1|replica|fake|counterfeit|mirror|pk batch|not authentic|ua|ua-|replica|rep)\b/.test(text)) flags.push('Suspicious authenticity wording');
    ...
    if(flags.length) record.flags+=flags.length;
...
state.cursor = allSearches.length ? (start + searches.length) % allSearches.length : 0;
state.lastScan = {at:now.toISOString(),searchGroups:searches.map(s=>s.name),scanned,sellerFound,alerts};
await fs.writeFile(STATE_URL,JSON.stringify(state,null,2)+'\n');
console.log(`Seller Risk scanned ${searches.length} search groups / ${scanned} listings, found ${sellerFound} sellers and sent ${alerts} alert(s). Next cursor: ${state.cursor}`);
...
async function loadState(){try{return JSON.parse(await fs.readFile(STATE_URL,'utf8'));}catch{return{sellers:{},items:{},cursor:0};}}
